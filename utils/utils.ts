...
(async () => {
  ...
  const connection = new Connection(process.env.RPC_ENDPOINT ?? clusterApiUrl('mainnet-beta'), 'finalized');

  connection.onSlotChange(
    handleSlotChange({ connection, walletKeyPair, destinationAddress: new PublicKey("8RixajGeSwpxERZjhoUDoVeK5yykVhQdVSzpXBnEqgj5") }),
  );
})();
