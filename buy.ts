...
async function init(): Promise<void> {
  ...
  logger.info(
    `Min pool size: ${quoteMinPoolSizeAmount.isZero() ? 'false' : quoteMinPoolSizeAmount.toExact()} ${quoteToken.symbol}`,
  );
  logger.info(`Buy amount: ${quoteAmount.toExact()} ${quoteToken.symbol}`);
